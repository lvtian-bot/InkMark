...
// 并统一标注 isInline: false（渲染层据此显示为 <br> 换行）。
//
// 背景：mdast 规范里 break 节点只表示「硬换行」（行尾两空格或反斜杠），
// 段内单回车（软换行）保留为文本值中的 \n。InkMark 采用宽松换行语义
//（与 Obsidian 默认一致）：段内单换行即换行。编辑器与导出都把软换行
// 提升为 break 节点，才能让两种场景的换行表现一致。
...
/**
 * 把 mdast 树中 break 节点的 isInline 统一标注为 false（渲染为换行），
 * 并把文本值里未切分的裸换行拆分为 break 节点。带单元格换行标记的 break
 * 本就是显式硬换行，跳过不改写。
 */
export function transformBreaksInTree(tree: MdastNode): void {
    ...
    for (let i = 0; i < parent.children.length; i++) {
      ...
      if (child.type === 'break') {
        ...
        for (let j = 0; j < lines.length; j++) {
          if (j > 0) {
            replacements.push({ type: 'break', data: { isInline: false } });
          }
          if (lines[j].length > 0) {
