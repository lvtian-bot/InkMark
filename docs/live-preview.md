...
2026-08-13 曾基于 ProseMirror 行首装饰（`Decoration.widget`）实现了轻量级块级浮现与行首 `#`/Backspace 升降级。但在实际使用中发现：

- 用户直觉普遍期望光标能够像编辑普通文字一样直接点入 `#` 或 `>` 字符内部进行选区、修改或删除；
- 装饰节点的 `contentEditable="false"` 特性使得鼠标点击会直接跳过符号落入正文，造成用户强烈的「标记浮现了但无法点击编辑」的认知冲突；
- 仅支持行首按特定键升降级的交互学习成本高，不符合自然直觉。

### 2. 清理决策

为避免半成品功能给用户带来困惑及引入长期维护负担，于 2026-08-16 将该功能从代码库完全移除（包括 `block-marker-reveal` 插件、设置项、样式及测试用例），并归档至 `docs/TODO.md` 的「暂不需要」分类。

### 3. 后续若重新立项的技术建议

若未来重新评估并决定支持完整的 Live Preview 标记编辑能力：

- **方案选型**：不应再使用行首装饰方案，必须采用真正的 **可编辑 NodeView**（参照 `plugins/frontmatter.ts` 模式）——在聚焦时将块节点展开为真实的单行/多行纯文本输入框，失焦时渲染回富文本；
- **历史代码参考**：可查阅 2026-08-13 至 2026-08-16 期间的 Git 历史提交获取装饰方案与按键处理的实现参考。
